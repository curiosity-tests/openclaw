...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  const tool: AnyAgentTool = {
    ...
    description: managementAuthority?.managementOnly
      ? 'Manage any existing automation on this Gateway as the authenticated Control UI administrator. Actions: list [includeDisabled,limit,offset] (compact summaries; follow nextOffset); get jobId (exact schedule, payload, and delivery details); update jobId job (partial patch, null clears); run jobId (runMode:"force" runs now); remove jobId. Creator attribution and scheduled execution policy stay intact. Use the Automations page for other actions.'
